...
Lo ideal para crear este entorno es usar una máquina virtual recien instalada y actualizada, clonar el repositorio de nuestros dotfiles, crear un snapshot y a partir de ahi ir probando los cambios que hagamos.

Aun asi, dependiendo de que *DE* queramos probar, una VM puede dar mas de un quebradero de cabeza, si disponemos de otro equipo totalmente separado, es buena idea configurarlo ahi.

## Estructura del proyecto

Vamos a tener una estructura similar a esta:
...
La mayoría de directorios como `i3`, `hypr`, `nvim`, o `bash` son donde almacenaremos nuestros `dotfiles`, siguiendo la estructura que veremos mas adelante para que `stow` funcione correctamente.

Los otros directorios y archivos que más nos interesan ahora mismo y que no tienen que ver con los `dotfiles` directamente son:

- `scripts`: Aqui es donde crearemos nuestros `sh` para instalar las dependencias de cada entorno que queramos.
- `applications`: 
- `user-scripts`: Scripts de bash que queremos poder ejecutar directamente desde el sistema como si fueran un binario, no confundir con el anterior.
- `Makefile`: El archivo que nos permite orquestar que scripts y comandos se lanzan para recrear cada entorno, y en que orden queremos ejecutarlos.

## Scripts

Vamos a crear un script genérico que queremos ejecutar y **que no depende de la distro**, por ejemplo:
...
Aunque podríamos usar el `dotfile` de git, esto nos permite que cada usuario configure su propio perfil de git en el sistema.

> Una alternativa mucho mas escalable en algunas aplicaciones que no tienen comandos directos para editar los dotfiles sería usar `templates` con el comando `envsubst`.

El resto de scripts que **si dependen de que distro estamos usando** deberian llevar un nombre con un patrón que ayuden a identificarlos fácilmente, por ejemplo:

- `fedora-common.sh`: Herramientas que independientemente del *DE* que estemos usando queremos tener, por ejemplo, nvim, lazyit, docker...
- `fedora-hyprland.sh`: Paquetes exclusivos del entorno hyprland
- `fedora-i3.sh`: Paquetes exclusivos del entorno i3
- `debian-i3.sh`: Paquetes exclusivos del entorno i3

Separarlos por distro nos permite tener una forma simple de manejar múltiples distros a la vez, manteniendo el mismo **DE**, y apreciar las diferencias entre ellos de forma clara.

Podemos crear scripts adicionales con herramientas que quizá no queremos en todas nuestras máquinas, por ejemplo `fedora-hacking.sh` puede ocuparse de instalar herramientas de ciberseguridad y pentesting, `fedora-gamin.sh` de instalar drivers y aplicaciones como *steam*, `debian-creative.sh` de paquetes relacionados con diseño gráfico y arte.

Lo importante en realidad es separarlos por distro e ir comprobando que funcionan.
